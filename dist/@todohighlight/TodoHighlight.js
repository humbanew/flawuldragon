...
class TodoHighlight {
    ...
    todoHighlight_createStatusBarItem(numNotations) {
        let todoHighlightStatusBarItem = constants_js_1.constants.statusBar.positions.posF;
        if (numNotations == 0) {
    ...
    todoHighlight_activate(context) {
        try {
            ...
            context.subscriptions.push(vscode.commands.registerCommand(constants_js_1.constants.commands.todoHighlight.release.fdToggleTodoHighlight, () => {
                settings
            ...
            context.subscriptions.push(vscode.commands.registerCommand(constants_js_1.constants.commands.todoHighlight.release.fdListAnnotations, () => {
                if (keywordsPattern?.trim()) {
            ...
            context.subscriptions.push(vscode.commands.registerCommand(constants_js_1.constants.commands.todoHighlight.release.fdOutputPanel, () => {
                var annotationList = workspaceState.get("annotationList", []);
                new TodoHighlight().todoHighlight_showOutputChannel(annotationList);
            }));
            const statusBarCommand = constants_js_1.constants.commands.todoHighlight.release.fdTodoHighlight;
