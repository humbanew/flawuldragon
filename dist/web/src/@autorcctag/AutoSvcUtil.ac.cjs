        ...
        do {
            ...
            if (char === '<') {
                if (scanner.stream.peekRight(1) === '/') {
                    ...
                    scanner.state = 2 /* EARCCTSvcHSScannerStateFast.AfterOpeningEndTag */;
                    const token = scanner.scan();
                    if (token !== 9 /* EARCCTSvcHSTokenTypeFast.EndTag */) {
                        return undefined;
                    }
                ...
                scanner.state = 1 /* EARCCTSvcHSScannerStateFast.AfterOpeningStartTag */;
                const token = scanner.scan();
                if (token !== 6 /* EARCCTSvcHSTokenTypeFast.StartTag */) {
                    return undefined;
                }
                const tokenText = scanner.getTokenText();
