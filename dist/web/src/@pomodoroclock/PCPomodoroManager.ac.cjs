...
const enums_cjs_1 = require("./enums.cjs");
const PCPomodoro_ac_cjs_1 = require("./PCPomodoro.ac.cjs");
const Vanilla_cjs_1 = require("../@vanilla/Vanilla.cjs");
...
class PCPomodoroManager {
    ...
    constructor(vscodeContext) {
        ...
        if (!this._clockBarText) {
            this._clockBarText = constants_cjs_1.constants.statusBar.positions.posH;
            this._typeBarText = constants_cjs_1.constants.statusBar.positions.posG;
            this._clockBarText.command =
                constants_cjs_1.constants.commands.pomodoroClock.release.fdToggleCurrentPomodoroCountdown;
        ...
        this._commandMap = {
            start: {
                link: vscode.Uri.parse(`command:${constants_cjs_1.constants.commands.pomodoroClock.release.fdPomodoroStartClock}`),
                imgSrc: vscode.Uri.joinPath(this.vscodeContext.extensionUri, "assets/pomodoroClock/", "start.svg"),
            },
            pause: {
                link: vscode.Uri.parse(`command:${constants_cjs_1.constants.commands.pomodoroClock.release.fdPomodoroPauseClock}`),
                imgSrc: vscode.Uri.joinPath(this.vscodeContext.extensionUri, "assets/pomodoroClock/", "pause.svg"),
            },
            continue: {
                link: vscode.Uri.parse(`command:${constants_cjs_1.constants.commands.pomodoroClock.release.fdPomodoroContinueClock}`),
                imgSrc: vscode.Uri.joinPath(this.vscodeContext.extensionUri, "assets/pomodoroClock/", "continue.svg"),
            },
            restart: {
                link: vscode.Uri.parse(`command:${constants_cjs_1.constants.commands.pomodoroClock.release.fdPomodoroRestartClock}`),
                imgSrc: vscode.Uri.joinPath(this.vscodeContext.extensionUri, "assets/pomodoroClock/", "restart.svg"),
            },
            reset: {
                link: vscode.Uri.parse(`command:${constants_cjs_1.constants.commands.pomodoroClock.release.fdPomodoroResetClock}`),
        ...
        let clockStatusBarCommand = constants_cjs_1.constants.commands.pomodoroClock.release.fdClockStatusBar;
        let typeStatusBarCommand = constants_cjs_1.constants.commands.pomodoroClock.release.fdTypeStatusBar;
    ...
    setTypeStatusBar() {
        ...
        if (this.currentPomodoro.type) {
            if (this.currentPomodoro.type === enums_cjs_1.EPCPomodoroType.Work) {
                countNumberPart += "(" + this._pomodoroCount + ")";
            }
            else if (this.currentPomodoro.type === enums_cjs_1.EPCPomodoroType.Break ||
                this.currentPomodoro.type === enums_cjs_1.EPCPomodoroType.LongBreak) {
    ...
    setClockStatusBar() {
        const btns = [];
        if (this.currentPomodoro.status === enums_cjs_1.EPCPomodoroStatus.None ||
            this.currentPomodoro.status === enums_cjs_1.EPCPomodoroStatus.Done) {
            btns.push(`<a href="${this._commandMap.start.link}"><img src="${this._commandMap.start.imgSrc}" /></a>`);
        }
        if (this.currentPomodoro.status === enums_cjs_1.EPCPomodoroStatus.Running) {
            btns.push(`<a href="${this._commandMap.pause.link}"><img src="${this._commandMap.pause.imgSrc}" /></a>`);
        }
        else if (this.currentPomodoro.status === enums_cjs_1.EPCPomodoroStatus.Paused) {
     ...
     */
    async tick() {
        if (this.currentPomodoro.status === enums_cjs_1.EPCPomodoroStatus.Done) {
            if (this.currentPomodoro.type === enums_cjs_1.EPCPomodoroType.Work) {
                const isLongBreack = !(this._pomodoroCount % this.repeat);
                this._pomodoroCount++;
                this.currentPomodoro.type = isLongBreack
                    ? enums_cjs_1.EPCPomodoroType.LongBreak
                    : enums_cjs_1.EPCPomodoroType.Break;
                ...
                vscode.window
                    ...
                    if (select === breakText) {
                        ...
                        this.start(enums_cjs_1.EPCPomodoroType.Work);
                    }
                });
            }
            else if (this.currentPomodoro.type === enums_cjs_1.EPCPomodoroType.Break) {
                this._breakCount++;
                this.currentPomodoro.type = enums_cjs_1.EPCPomodoroType.Work;
                vscode.window
                    .showInformationMessage("Break is over.", "Next work", "Continue break")
                    .then((select) => {
                    if (select === "Continue break") {
                        this.start(enums_cjs_1.EPCPomodoroType.Break);
                    }
                    else if (select === "Next work") {
                        this.start(enums_cjs_1.EPCPomodoroType.Work);
                    }
            ...
            else if (this.currentPomodoro.type === enums_cjs_1.EPCPomodoroType.LongBreak) {
                this._breakCount++;
                this.currentPomodoro.type = enums_cjs_1.EPCPomodoroType.Work;
                vscode.window
                    .showInformationMessage("Break is over.", "Next work", "Continue break")
                    .then((select) => {
                    if (select === "Continue break") {
                        this.start(enums_cjs_1.EPCPomodoroType.LongBreak);
                    }
                    else if (select === "Next work") {
                        this.start(enums_cjs_1.EPCPomodoroType.Work);
                    }
     ...
     */
    start(type = enums_cjs_1.EPCPomodoroType.Work) {
