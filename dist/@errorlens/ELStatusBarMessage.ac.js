...
class ELStatusBarMessage {
    ...
    updateText(editor, groupedDiagnostics) {
        ...
        if (this.messageType === 'activeLine') {
            ...
            outerLoop: for (const lineNumber of sortedLineNumbers) {
                ...
                for (const diag of diagnosticsAtLine) {
                    if (ELExtUtils_ac_js_1.ELExtUtils.prototype.isSeverityEnabled(diag.severity)) {
                        diagnostic = diag;
        ...
        let message = ELExtUtils_ac_js_1.ELExtUtils.prototype.diagnosticToInlineMessage(ErrorLens_js_1.$config.statusBarMessageTemplate || ErrorLens_js_1.$config.messageTemplate, diagnostic, numberOfDiagnosticsOnThatLine);
        if (ErrorLens_js_1.$config.removeLinebreaks) {
            message = ELUtils_ac_js_1.ELUtils.prototype.replaceLinebreaks(message, ErrorLens_js_1.$config.replaceLinebreaksSymbol);
        }
    ...
    makeTooltip(diagnostic, lintFilePaths) {
        const markdownHover = ELHover_ac_js_1.ELHover.prototype.createHoverForDiagnostic({
            diagnostic,
            buttonsEnabled: true,
            messageEnabled: true,
