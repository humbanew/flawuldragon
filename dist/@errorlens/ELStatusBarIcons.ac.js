...
class ELStatusBarIcons {
    ...
    updateText() {
        ...
        let errorCount = 0;
        let warningCount = 0;
        const allDiagnostics = ELExtUtils_ac_js_1.ELExtUtils.prototype.getDiagnostics({
    ...
    makeTooltip(allDiagnostics, type) {
        ...
        for (const diagWithUri of allDiagnostics) {
            ...
            if (diagnostics.length) {
                markdown.appendMarkdown(`**${ELUtils_ac_js_1.ELUtils.prototype.basename(uri.path)}**\n\n`);
            }
            for (const diag of diagnostics) {
    ...
    clearBackground(statusBarType) {
        if (statusBarType === 'error') {
            this.errorStatusBarItem.backgroundColor = new vscode.ThemeColor("statusBarItem.warningBackground");
        }
        else if (statusBarType === 'warning') {
