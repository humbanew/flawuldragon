...
import { HCSSettings } from './HCSSettings';
import { HCSProvider } from './HCSProvide';
  ...
  public activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
      ...
      vscode.workspace.onDidSaveTextDocument(async (document) => {
        ...
        await this.validate(document, HCSSetAutoValidation.SAVE);
      }),
      vscode.workspace.onDidOpenTextDocument(async (document) => {
        await this.validate(document, HCSSetAutoValidation.ALWAYS);
      }),
      vscode.workspace.onDidChangeTextDocument(async (event) => {
        if (event.contentChanges.length > 0) {
          await this.validate(event.document, HCSSetAutoValidation.ALWAYS);
        }
      }),
      ...
      vscode.commands.registerCommand(
        Global.htmlCssSupport.comandos.validate,
        async (type: HCSSetAutoValidation | undefined) => {
          const editor = vscode.window.activeTextEditor;
          if (editor) {
            await this.validate(editor.document, type);
          }
        }
      ),
      vscode.commands.registerCommand(Global.htmlCssSupport.comandos.clear, () => HCSProvider.prototype.clear())
    );
