...
import { EPCPomodoroType, EPCPomodoroStatus } from "./enums";
import { TPCCommandStatus } from "./declares";
import { PCPomodoro } from "./PCPomodoro";
...
export class PCPomodoroManager {
  ...
  constructor(public vscodeContext: vscode.ExtensionContext) {
    ...
    this._commandMap = {
      start: {
        link: vscode.Uri.parse(
          `command:${Global.pomodoroClock.comandos["start-pomodoro"]}`,
        ),
      ...
      pause: {
        link: vscode.Uri.parse(
          `command:${Global.pomodoroClock.comandos["pause-pomodoro"]}`,
        ),
        imgSrc: vscode.Uri.joinPath(
          this.vscodeContext.extensionUri,
          "assets/pomodoroClock/",
      ...
      continue: {
        link: vscode.Uri.parse(
          `command:${Global.pomodoroClock.comandos["continue-pomodoro"]}`,
        ),
        imgSrc: vscode.Uri.joinPath(
          this.vscodeContext.extensionUri,
          "assets/pomodoroClock/",
      ...
      restart: {
        link: vscode.Uri.parse(
          `command:${Global.pomodoroClock.comandos["restart-pomodoro"]}`,
        ),
        imgSrc: vscode.Uri.joinPath(
          this.vscodeContext.extensionUri,
          "assets/pomodoroClock/",
      ...
      reset: {
        link: vscode.Uri.parse(
          `command:${Global.pomodoroClock.comandos["reset-pomodoro"]}`,
        ),
        imgSrc: vscode.Uri.joinPath(
          this.vscodeContext.extensionUri,
          "assets/pomodoroClock/",
    ...
    let clockStatusBarCommand = Global.pomodoroClock.comandos["clock-status-bar"];
    let typeStatusBarCommand = Global.pomodoroClock.comandos["type-status-bar"];
    FDVanilla.prototype.interruptorStatusBarConstructor(this._clockBarText, clockStatusBarCommand);
    FDVanilla.prototype.interruptorStatusBarConstructor(this._typeBarText, typeStatusBarCommand);
  }
  ...
  private setParamsFromConfig() {
    ...
    this.workTime = config.get<number>("workTime", 25);
    this.breakTime = config.get<number>("breakTime", 5);
    this.longBreakTime = config.get<number>("longBreakTime", 15);
    this.isCountDown = config.get<boolean>("isCountDown", true);
    this.repeat = config.get<number>("repeat", 4);
  }
