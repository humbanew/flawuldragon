...
export class ARTSvcUtil {
  ...
  public getNextClosingTagName: (
    scanner: IARCCTSvcHSScannerFast,
    ...
    do {
      ...
      if (char === '<') {
        if (scanner.stream.peekRight(1) === '/') {
          ...
          scanner.state = EARCCTSvcHSScannerStateFast.AfterOpeningEndTag;
          const token = scanner.scan();
          if (token !== EARCCTSvcHSTokenTypeFast.EndTag) {
            return undefined;
          }
        ...
        scanner.state = EARCCTSvcHSScannerStateFast.AfterOpeningStartTag;
        const token = scanner.scan();
        if (token !== EARCCTSvcHSTokenTypeFast.StartTag) {
          return undefined;
        }
        const tokenText = scanner.getTokenText();
