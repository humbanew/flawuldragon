...
import {
  EARCCTSvcHSScannerStateFast,
  EARCCTSvcHSTokenTypeFast
} from './enums';

export class ARTSvcHtmlScanner {
  /**
   ...
   */
  public htmlTagNameRE = /^[!:\w\$]((?![>\/])[\S])*/;

  /**
   * Empty html tag, e.g. `< ></>`
   */
  public htmlTagNameEmptyRE = /^\s+/;
  ...
  public createScannerFast({
    ...
    const stream = new ARTSvcHtmlScanner(
      input,
      initialOffset,
      matchingTagPairs
    );
    let state: EARCCTSvcHSScannerStateFast = initialState;
    ...
    let nextElementName = (): string | undefined => {
    ...
    function scan(): TokenTypeFast {
      ...
      if (stream.eos()) {
        return EARCCTSvcHSTokenTypeFast.EOS;
      }
      switch (state) {
        case EARCCTSvcHSScannerStateFast.AfterOpeningEndTag:
          const tagName = nextElementName();
          if (tagName) {
            state = EARCCTSvcHSScannerStateFast.WithinEndTag;
            return EARCCTSvcHSTokenTypeFast.EndTag;
          } else if (stream.peekRight(0) === '>') {
            state = EARCCTSvcHSScannerStateFast.WithinEndTag;
            return EARCCTSvcHSTokenTypeFast.EndTag;
          }
          return EARCCTSvcHSTokenTypeFast.Unknown;

        case EARCCTSvcHSScannerStateFast.AfterOpeningStartTag:
          lastTagName = nextElementName();
          if (lastTagName !== undefined) {
            if (lastTagName === '') {
              tokenOffset = stream.position;
            }
            state = EARCCTSvcHSScannerStateFast.WithinStartTag;
            return EARCCTSvcHSTokenTypeFast.StartTag;
          }
          // this is a tag like "<>"
          if (stream.peekRight() === '>') {
            state = EARCCTSvcHSScannerStateFast.WithinStartTag;
          ...
          state = EARCCTSvcHSScannerStateFast.WithinContent;
          return scan();
        default:
          break;
      }
