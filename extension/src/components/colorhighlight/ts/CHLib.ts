...
export class CHLib {
  ...
  // dynamic contrast
  public getColorContrast(color: any) {
    ...
    let luminance = this.relativeLuminance(r, g, b);
    ...
    let contrastWhite = this.contrastRatio(luminance, luminanceWhite);
    let contrastBlack = this.contrastRatio(luminance, luminanceBlack);
   ...
   */
  public contrastRatio(l1: number, l2: number) {
   ...
   *                   and 1.0 (inclusive).
   */
  public relativeLuminance(r8: any, g8: any, b8: any) {
    const bigR = this.srgb8ToLinear(r8);
    const bigG = this.srgb8ToLinear(g8);
    const bigB = this.srgb8ToLinear(b8);
  ...
  // sass importer
  public parseImports(options: any) {
    ...
    if (!this._map.has(color)) {
      ...
      switch (this.options.markerType) {
        ...
        default:
          ...
          rules.color = this.getColorContrast(color);
          rules.border = `3px solid ${color}`;
