...
import { $state, $config } from './ErrorLens.js';
...
export class ELExtUtils {
  ...
  public prepareMessage({ template, diagnostic, lineProblemCount, removeLinebreaks, replaceLinebreaksSymbol }: IELPrepareMessageArg): string {
    ...
    return ELUtils.prototype.truncateString(removeLinebreaks ? ELUtils.prototype.replaceLinebreaks(templated, replaceLinebreaksSymbol) : templated, $config.messageMaxChars);
  }
