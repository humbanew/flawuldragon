...
import { ELVSCodeUtils } from './ELVSCodeUtils.ac.js';
import { EELConstants, EELCommandId } from './enums.js';
import { $config, $state } from './ErrorLens.js';
...
export class ELCommands {
  ...
  public codeLensOnClickCommand(diagnostic: vscode.Diagnostic): void {
    switch ($config.codeLensOnClick) {
      case 'showProblemsView':
        ...
        break;
      case 'showQuickFix':
        ELVSCodeUtils.prototype.setCaretInEditor({
  ...
  public copyProblemCodeCommand(arg: { code: string | undefined } | undefined): void {
    ...
    const diagnosticAtActiveLineNumber = ELExtUtils.prototype.getDiagnosticAtLine(editor.document.uri, editor.selection.active.line);

    if (!diagnosticAtActiveLineNumber) {
      return;
    }

    const codeAsString = ELExtUtils.prototype.getDiagnosticCode(diagnosticAtActiveLineNumber);
    if (!codeAsString) {
      return;
    }
  ...
  public copyProblemMessageCommand(editor: vscode.TextEditor): void {
    const groupedDiagnostics = ELExtUtils.prototype.groupDiagnosticsByLine(vscode.languages.getDiagnostics(editor.document.uri));
  ...
  public disableLineCommand(diagnostic: vscode.Diagnostic | undefined): void {
    ...
    if (diagnostic) {
      ...
      const diagnosticAtActiveLine = ELExtUtils.prototype.getDiagnosticAtLine(editor.document.uri, editor.selection.active.line);
    ...
    let comment = ELExtUtils.prototype.diagnosticToInlineMessage(template, diagnostic, 0);
    ...
    if (isTheSameLine) {
      ...
      comment = `${ELVSCodeUtils.prototype.getIndentationAtLine(editor.document, lineNumber)}${comment}\n`;
    }
  ...
  protected async showNoCommentSpecifiedForSource(source: string): Promise<void> {
    ...
    if (pressedButton === showSettingsButton) {
      ELVSCodeUtils.prototype.openSettingGuiAt(`@ext:${EELConstants.ExtensionId} ${'disableLineComments' satisfies keyof TELExtensionConfig}`);
    }
  }

  public async excludeProblemCommand(diagnostic: vscode.Diagnostic): Promise<void> {
    const code = ELExtUtils.prototype.getDiagnosticCode(diagnostic);
    ...
    const sourceCodeString = ELExtUtils.prototype.diagnosticToSourceCodeString(source, code);

    if ($config.excludeBySource.includes(sourceCodeString)) {
      return;
    }

    await ELVSCodeUtils.prototype.updateGlobalSetting(`${EELConstants.SettingsPrefix}.${'excludeBySource' satisfies keyof TELExtensionConfig}`, [
  ...
  protected async showExcludeByMessageNotification(message: string, diagnostic: vscode.Diagnostic): Promise<void> {
    ...
    await ELVSCodeUtils.prototype.updateGlobalSetting(`${EELConstants.SettingsPrefix}.${'exclude' satisfies keyof TELExtensionConfig}`, [
      ...$config.exclude,
      messageToExclude,
    ]);
  ...
  protected async showCompletionByMessageNotification(messageToExclude: string): Promise<void> {
    ...
    if (pressedButton === openSettingsButton) {
      ELVSCodeUtils.prototype.openSettingGuiAt(`@ext:${EELConstants.ExtensionId} ${EELConstants.SettingsPrefix}.${'exclude' satisfies keyof TELExtensionConfig}`);
    }
  }
  ...
  protected async showCompletionNotification(sourceCodeString: string): Promise<void> {
    ...
    if (pressedButton === openSettingsButton) {
      ELVSCodeUtils.prototype.openSettingGuiAt(`@ext:${EELConstants.ExtensionId} ${'excludeBySource' satisfies keyof TELExtensionConfig}`);
    }
  }
  ...
  public async findLinterRuleDefinitionCommand(args?: IELRuleDefinitionArgs): Promise<void> {
    ...
    if (!args) {
      ...
      if (diagnostic) {
        code = ELExtUtils.prototype.getDiagnosticCode(diagnostic);
    ...
    for (const linterFilePath of linterFilePaths) {
      const fileContents = await ELVSCodeUtils.prototype.readFileVscode(linterFilePath.fsPath);
    ...
    if (!ruleMatchResults.length) {
      ...
      if (linterFilePaths.length === 1) {
        ...
        const editor = await ELVSCodeUtils.prototype.openFileInVscode(linterFilePaths[0]);
        ELVSCodeUtils.prototype.revealLine(editor, 0);
      } else {
        const pickedFile = await vscode.window.showQuickPick(linterFilePaths.map(filePathUri => filePathUri.fsPath), {
          title: message,
        });
        if (pickedFile) {
          ELVSCodeUtils.prototype.openFileInVscode(pickedFile);
        }
      }
      return;
    }
  ...
  protected async showNotificationWithOpenSettingsButton(message: string): Promise<void> {
    ...
    if (pressedButton === openLinterSetting) {
      ELVSCodeUtils.prototype.openSettingGuiAt(`@ext:${EELConstants.ExtensionId} ${'lintFilePaths' satisfies keyof TELExtensionConfig}`);
    }
  }
  ...
  protected async openAndReveal(pathOrUri: vscode.Uri | string, contents: string, needle: string): Promise<void> {
    const textEditor = await ELVSCodeUtils.prototype.openFileInVscode(pathOrUri);
    const lineNumber = this.getLineNumberForMatch(contents, needle);
    if (lineNumber) {
      ELVSCodeUtils.prototype.revealLine(textEditor, lineNumber);
    }
  }
  ...
  protected tryToFindDiagnosticFromActiveEditor(): vscode.Diagnostic | undefined {
    ...
    const groupedDiagnostics = ELExtUtils.prototype.groupDiagnosticsByLine(diagnosticsForUri);
  ...
  public searchForProblemCommand(diagnostic: vscode.Diagnostic | undefined): void {
    ...
    const query = ELExtUtils.prototype.diagnosticToInlineMessage($config.searchForProblemQuery, diagnostic, 0);
  ...
  public selectProblemCommand(editor: vscode.TextEditor): void {
    ...
    if ($config.selectProblemType === 'closestProblem') {
      ...
      diagnostic = ELExtUtils.prototype.getDiagnosticAtLine(editor.document.uri, editor.selection.active.line);
    }
  ...
  protected async showResultNotification(message: string): Promise<void> {
    ...
    ELVSCodeUtils.prototype.openSettingGuiAt(`@ext:${EELConstants.ExtensionId} ${'excludeWorkspaces' satisfies keyof TELExtensionConfig}`);
  }
