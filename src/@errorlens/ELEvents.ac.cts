...
import { ELCustomDelay } from './ELCustomDelay.ac.cjs';
import { ELDecorations } from './ELDecorations.ac.cjs';
import { ELExtUtils } from './ELExtUtils.ac.cjs';
import { ELNewDelay } from './ELNewDelay.ac.cjs';
        ...
        if (this.caretMovedToAnotherLine(e.selections, lastPositionLine)) {
          ...
          if (this.extUtils.shouldShowStatusBarMessage()) {
            $state.statusBarMessage.updateText(
              e.textEditor,
              this.extUtils.groupDiagnosticsByLine(vscode.languages.getDiagnostics(e.textEditor.document.uri)),
            );
          }
