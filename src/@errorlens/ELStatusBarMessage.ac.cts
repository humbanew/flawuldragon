...
import { EELCommandId } from './enums.js';
...
export class ELStatusBarMessage {
  ...
  public updateText(editor: vscode.TextEditor, groupedDiagnostics: TELGroupedByLineDiagnostics): void {
    ...
    let message = this.extUtils.diagnosticToInlineMessage(
    ...
    if ($config.removeLinebreaks) {
      message = this.utils.replaceLinebreaks(message, $config.replaceLinebreaksSymbol);
    }
