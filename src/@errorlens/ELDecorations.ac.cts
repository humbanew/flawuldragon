...
import { ELHover } from "./ELHover.ac.cjs";
import { EELConstants } from "./enums.js";
import { ELGutter } from './ELGutter.ac.cjs';
import { ELMultilineDecoration } from './ELMultilineDecoration.ac.cjs';
...
export class ELDecorations {
  ...
  protected hover = new ELHover;
  protected gutter = new ELGutter;
  protected multilineDecoration = new ELMultilineDecoration;
  ...
  public setDecorationStyle(context: vscode.ExtensionContext): void {
    ...
    if (this.extUtils.shouldShowGutterIcons()) {
      gutter = this.gutter.getGutterStyles(context);
    ...
    if ($config.followCursor === 'closestProblemMultiline' ||
      ...
      this.multilineDecoration.createMultilineDecorations();
    }
    ...
    if (!this.extUtils.shouldShowInlineMessage()) {
  ...
  public doUpdateDecorations(editor: vscode.TextEditor, groupedDiagnostics: TELGroupedByLineDiagnostics, range?: vscode.Range): void {
    ...
    for (const key in groupedDiagnostics) {
      ...
      if (this.extUtils.shouldShowInlineMessage()) {
        message = this.extUtils.prepareMessage({
          diagnostic,
      ...
      const decInstanceRenderOptions: vscode.DecorationInstanceRenderOptions = {
        after: {
          ...
          textDecoration: this.extUtils.shouldAlign() ? `${this.textDecorationStyleString};${alignMarginStyle}` : undefined,
        },
      };
      ...
      const diagnosticDecorationOptions: vscode.DecorationOptions = {
        ...
        hoverMessage: this.hover.createHoverForDiagnostic({
          diagnostic,
    ...
    if ($state.renderGutterIconsAsSeparateDecoration) {
      this.gutter.doUpdateGutterDecorations(editor, groupedDiagnostics);
    }
  ...
  public updateDecorationsForUri({
    ...
    this.doUpdateDecorations(editor, groupedDiagnostics ?? this.extUtils.groupDiagnosticsByLine(vscode.languages.getDiagnostics(uri)), range);
  }
