...
function getIcons(lightTheme: Theme, darkTheme: Theme) {
  const icons: { [key: string]: { light: svg.INode | null; dark: svg.INode | null; iconName: string } } = {};
...
function buildAutoIcons(
  ...
  for (const iconDefinitionKey in icons) {
    ...
    if ((icon.light != null ) && (icon.dark != null)) {
      autoIcon = getAutoIconAst(icon.light, icon.dark);
...
function fixIconPaths(theme: Theme, themePath: string) {
  for (const iconDefinitionKey in theme.iconDefinitions) {
    const iconDefinition = theme.iconDefinitions[iconDefinitionKey as keyof typeof theme.iconDefinitions];
