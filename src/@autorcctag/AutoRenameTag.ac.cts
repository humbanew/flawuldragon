...
export class AutoRenameTag {
  ...
  public assertDefined: <T>(value: T) => asserts value is NonNullable<T> = val => {
  ...
  public autoRenameTagRequestType = new RequestType<IARCCTParams, IARCCTResult[], any>(
    '$/auto-rename-tag'
  );
  ...
  public askServerForAutoCompletionsElementRenameTag: (
    ...
    tags: IARCCTTag[]
  ) => Promise<IARCCTResult[]> = async (languageClientProxy, document, tags) => {
  ...
  public applyResults: (results: IARCCTResult[]) => Promise<void> = async results => {
