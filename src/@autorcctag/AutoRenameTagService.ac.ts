import { EARCCTSvcHSScannerStateFast } from './enums.js';
import { ARTSvcHtmlScanner } from './AutoSvcHtmlScanner.ac.js';
import { ARTSvcUtil } from './AutoSvcUtil.ac.js';

export class AutoRenameTagService {
  ...
  public doAutoRenameTag: (
    ...
    const scanner = ARTSvcHtmlScanner.prototype.createScannerFast({
      input: text,
      initialOffset: 0,
      initialState: EARCCTSvcHSScannerStateFast.WithinContent,
      matchingTagPairs
    });
    if (newWord.startsWith('</')) {
      ...
      const parent = ARTSvcUtil.prototype.getPreviousOpeningTagName(
        scanner,
      ...
      const nextClosingTag = ARTSvcUtil.prototype.getNextClosingTagName(
        scanner,
        scanner.stream.position,
        isSelfClosingTag,
        isReact
      ...
      const previousOpenTag = ARTSvcUtil.prototype.getPreviousOpeningTagName(
        scanner,
        offset,
        isSelfClosingTag,
        isReact
