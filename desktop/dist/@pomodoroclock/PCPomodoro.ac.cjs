...
class PCPomodoro {
    ...
    get totalTime() {
        let totalTime = 0;
        if (this.type === enums_cjs_1.EPCPomodoroType.Work) {
            totalTime = this.workTime;
        }
        else if (this.type === enums_cjs_1.EPCPomodoroType.Break) {
            totalTime = this.breakTime;
        }
        else if (this.type === enums_cjs_1.EPCPomodoroType.LongBreak) {
    ...
    constructor(workTime = 25 * 60, breakTime = 5 * 60, longBreakTime, isCountDown = true, type = enums_cjs_1.EPCPomodoroType.Work) {
        ...
        this._type = type;
        this._status = enums_cjs_1.EPCPomodoroStatus.None;
    }
    // private methods
    /**
     ...
     */
        ...
        this.timer.reset();
        this._status = enums_cjs_1.EPCPomodoroStatus.Done;
    }
    /**
     ...
     */
    tick() {
        this._status = enums_cjs_1.EPCPomodoroStatus.Running;
        this._timer.start(async () => {
            // console.log('tick', Math.random())
            // stop the timer if no second left
            const isDone = (this.type === enums_cjs_1.EPCPomodoroType.Work &&
                this.timer.accumulateTime === this.workTime) ||
                (this.type === enums_cjs_1.EPCPomodoroType.Break &&
                    this.timer.accumulateTime === this.breakTime) ||
                (this.type === enums_cjs_1.EPCPomodoroType.LongBreak &&
     ...
     */
    continue() {
        if (this.status === enums_cjs_1.EPCPomodoroStatus.Paused) {
            this.tick();
        }
    }
    /**
    ...
    pause() {
        this.stop();
        this._status = enums_cjs_1.EPCPomodoroStatus.Paused;
    }
    /**
    ...
    reset() {
        this._status = enums_cjs_1.EPCPomodoroStatus.None;
        this._type = enums_cjs_1.EPCPomodoroType.Work;
        this.timer.reset();
    }
    /**
    ...
    dispose() {
        this.stop();
        this.type = enums_cjs_1.EPCPomodoroType.Work;
        this._status = enums_cjs_1.EPCPomodoroStatus.Paused;
    }
}
