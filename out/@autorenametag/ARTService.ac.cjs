...
const ARTHTMLScannerService_ac_cjs_1 = require("./ARTHTMLScannerService.ac.cjs");
class ARTService {
    doAutoRenameTag = (text, offset, newWord, oldWord, languageId) => {
        ...
        const scanner = ARTHTMLScannerService_ac_cjs_1.ARTHTMLScannerService.call.prototype.createScannerFast({
            input: text,
            initialOffset: 0,
            initialState: 0 /* EARTSScannerStateFast.WithinContent */,
            matchingTagPairs
        });
        if (newWord.startsWith('</')) {
            ...
            const parent = ARTHTMLScannerService_ac_cjs_1.ARTHTMLScannerService.call.prototype.getPreviousOpeningTagName(scanner, scanner.stream.position, isSelfClosingTag, isReact);
            if (!parent) {
                return undefined;
            }
            ...
            const nextClosingTag = ARTHTMLScannerService_ac_cjs_1.ARTHTMLScannerService.call.prototype.getNextClosingTagName(scanner, scanner.stream.position, isSelfClosingTag, isReact);
            if (!nextClosingTag) {
                return undefined;
            }
            ...
            const previousOpenTag = ARTHTMLScannerService_ac_cjs_1.ARTHTMLScannerService.call.prototype.getPreviousOpeningTagName(scanner, offset, isSelfClosingTag, isReact);
            if (previousOpenTag &&
