...
export class AutoCloseTag {
  public activate(context: vscode.ExtensionContext) {
    ...
    let closeTag = vscode.commands.registerCommand(
      'flawuldragon.auto-close-tag.closeTag',
      () => {
        this.insertCloseTag();
      }
    );
  ...
  protected insertAutoCloseTag(event: vscode.TextDocumentChangeEvent): void {
    ...
    let isRightAngleBracket = this.CheckRightAngleBracket(
      event.contentChanges[0]
    );
  ...
  protected CheckRightAngleBracket(
  ...
  protected CheckRightAngleBracketInVSCode_1_8(
    contentChange: vscode.TextDocumentContentChangeEvent
  ): boolean {
    return (
